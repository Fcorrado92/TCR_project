...
T_cells_filt_clono<-subset(T_cells_filt2, cells=keep, final_annotation%in%c("CD8+TN"
))
...
keep<-unique(rownames(df))
clono<-subset(endogenous_t_cells, cells=keep, final_annotation%in%c("CD8+TN"
))
...
res_turbogliph <- turboGliph::turbo_gliph(cdr3_sequences = final3, 
                                          n_cores = 8)
saveRDS(res_turbogliph, "~/TCR_project/res_turbogliph_cd8naive.rds")
